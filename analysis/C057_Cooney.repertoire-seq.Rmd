...
James further requested that cluster 6 be excluded as he suspects it to be 

> "comprised of CD8 T cells that have upregulated CD4 (and were thus included in the sort). They have unique upregulation of many CD8-specific genes (GNLY, GZMA, GZMK) this to comprise CD8 T-cells"

The up-regulation of these genes can be observed in Figure \@ref(fig:cd8-markers).

```{r cd8-markers, fig.cap = "Expression of CD8-specific genes by cluster."}
plotExpression(
  sce,
  features = c("GNLY", "GZMA", "GZMK"),
  x = "cluster",
  colour_by = "cluster") + 
  theme_cowplot() +
...
From this, we see hints that fewer clonotypes are found in infected cells than in uninfected cells, but this is not a consistent result^[Previously, when cluster 6 was included, this was quite a strong result. However, cluster 6 is the largest cluster and is enriched for infected cells, so by removing it we are possibly removing that signal.].
