---
title: "Dimensionality reduction of the Cooney (C057) memory CD4+ T-cell data set after ignoring 'unknown' samples"
description: |
author:
  - name: Peter Hickey 
...
knitr::opts_chunk$set(fig.path = "C057_Cooney_ignoring_unknowns_merge_files/")
```

# Setting up the data

We start from the preprocessed *SingleCellExperiment* object created in ['Preprocessing the Cooney (C057) memory CD4+ T-cell data set'](C057_Cooney_preprocess.html), subsetted to exclude the 'unknown' samples.
...
# NOTE: Need to re-do gene removal and normalization(?)
```
...
# Removal of monocytes / dendritic cells

There is a small cluster of cells that are almost only found in uninfected samples (left-side of Figure \@ref(fig:tsne-by-treatment)).

```{r tsne-by-treatment, fig.cap = "t-SNE plot constructed from the top PCs in the dataset. Each point represents a cell, coloured according to the `Treatment`.", fig.asp = 0.8}
ggplot(aes(x = x, y = y), data = tsne_df) +
  geom_point(aes(colour = Treatment), size = 0.5) +
...
Some inspection of the genes expressed by these cells strongly suggests that these are monocytes / dendritic cells and not T-cells (Figure \@ref(fig:tsne-monocytes)).

```{r tsne-monocytes, fig.cap = "t-SNE plot constructed from the top PCs in the dataset. Each point represents a cell, coloured to highlight the expression of selected monocyte / dendritic cell and T-cell marker genes.", fig.asp = 1.5, layout = "l-body"}
plot_grid(
  plotTSNE(sce, colour_by = "LYZ"),
  plotTSNE(sce, colour_by = "CD74"),
  plotTSNE(sce, colour_by = "CST3"),
  plotTSNE(sce, colour_by = "FGL2"),
  plotTSNE(sce, colour_by = "CD3D"),
  plotTSNE(sce, colour_by = "IL32"),
  plotTSNE(sce, colour_by = "LAT"),
  plotTSNE(sce, colour_by = "CD2"),
  ncol = 2)
```

These cells likely slipped through in the FACS gating and are not of interest to James, so we remove these cells and repeat the above steps to construct new t-SNE co-ordinates.

```{r}
j <- reducedDim(sce, "TSNE")[, 1] < -40
# NOTE: This is a crude sanity check that we've excluded the right cells.
stopifnot(sum(j) == 157)
sce <- sce[, !j]
...
Figure \@ref(fig:tsne-more-factors) shows a strong separation of the `Treated` samples from the `Untreated` samples.
...
The processed *SingleCellExperiment* object is available (see [`data/SCEs/C057_Cooney_ignoring_unknowns.merged.SCE.rds`](../data/SCEs/C057_Cooney_ignoring_unknowns.merged.SCE.rds)).
