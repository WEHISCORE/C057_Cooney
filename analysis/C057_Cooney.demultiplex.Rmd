...
We therefore focus on assigning cells to just the `mouse_1`, `mouse_3`, and `mouse_4` HTOs.
...
We use $t$-stochastic neighbour embedding (t-SNE) plots [@van2008visualizing] () and uniform manifold approximation and projection (UMAP) plots [@McInnes2018-hy] (Figure \@ref(fig:umap)) to visualise the HTO data in 2-dimensions, overlaying the assigned cluster labels.

Figure \@ref(fig:tsne) and Figure \@ref(fig:umap), in particular, show that five clusters are clearly distinguishable from each other and two additional clusters taht are more difficult to resolve.


```{r tsne, fig.cap = "t-SNE plot of the HTO data. Each point represents a cell with the colour explained in the legend."}
library(cowplot)
...
```{r umap, fig.cap = "UMAP plot of the HTO data. Each point represents a cell with the colour explained in the legend."}
...
Figure \@ref(fig:violin) also reveals that the distribution of expression is very tight (i.e. low variability) for these samples labelled uniquely with that HTO as well as in the samples that were labelled with all 5 HTOs (cluster 4).
In contrast, the samples in cluster 7 often have high expression of each HTO but this expression is much more variable than those samples in cluster 4.
...
Most of these unassigned droplets likely comprise true doublets and mis-assigned singlets from the cells labelled with all five HTOs.
We will keep these as their own category to see if these cells are transcriptomically distinct, which may indicate an issue with cell hashing for certain types of cells.
