...
On the basis of Figure \@ref(fig:mosty-doublets-hto-heatmap), we assign droplets in these clusters accordingly:
...
colData(sce) <- cbind(
  colData(sce),
  sample_map[match(sce$hto_cluster, sample_map$hto_cluster), ])
```
