...
The following table gives the number of cluster-specific marker genes ($FDR < 0.05$) for each cluster.

```{r}
n <- sapply(markers_uniquely_up, function(x) sum(x$FDR < 0.05))
knitr::kable(
  data.frame(
    cluster = names(markers_uniquely_up),
    n = sapply(markers_uniquely_up, function(x) sum(x$FDR < 0.05))),
  caption = "Number of cluster-specific genes for each cluster.")
most_unique_markers <- names(which.max(n))
```
...
From this was can observe that:

- Most clusters only have a small number of marker genes with strong logFCs compared to all other clusters.
- Cluster 3, comprised mostly of `Infected` cells, has very high and distinct expression of several key immune genes (e.g., *GNLY*, *CCL5*, *CST7*, *CD74*). Some of the only other cells with such high expression of these genes appear to be the `Infected` cells in cluster 7, suggesting that those cells in cluster 7 may be of similar cell type to cells in cluster 3 but distinguished by strong upregualation of an additional set of genes.
- Cluster 7, comprised of `Infected` and `Uninfected` cells in a roughly 50:50 split, has very high and distinct expression of genes that are not obviously immune genes.
- Cluster 8 has very high expression of *FOXP3*, a classic marker of regulatory T cells. Indeed, most cells in this cluster were annotated by `r BiocStyle::Biocpkg("SingleR")` as being `T cells, CD4+, memory TREG`. However, so too were many cells from other clusters that have almost no expression of *FOXP3*. This perhaps highlights both the power of using a multi-gene approach to annotation (such as that used by `r BiocStyle::Biocpkg("SingleR")`) and the limitations of relying on 'known' marker genes for cell type annotation.
...
Cluster `r most_unique_markers` has the the largest number of cluster-specific marker genes ($n =$ `r n[most_unique_markers]`)).
...
pheatmap(
  ...
  cluster_cols = FALSE,
  show_colnames = FALSE,
  # NOTE: These annotations are a little clunky to be useful.
  # annotation_row = data.frame(
  #   cycle = dplyr::case_when(
  #     grepl("^CCNA", features) ~ "S,G2",
  #     grepl("^CCNB", features) ~ "G2,M",
  #     grepl("^CCND", features) ~ "G0,*G1*,S,G2,M",
  #     grepl("^CCNE", features) ~ "G1/S"),
  #   row.names = features),
  annotation_col = data.frame(
    cluster = sce$cluster[o],
    Treatment = sce$Treatment[o],
...
We can infer that cluster `r g2_cluster` likely has more cells in S,G2 or G2/M than the other clusters, based on higher expression of some of the cyclin A and B genes.
...
We can infer that cluster `r g2_cluster` likely contains cells in S,G2 or G2/M (based on higher expression of some of the cyclin A and B genes) and that the remaining clusters are likely in G1 (based on fairly ubiquitous expression of cyclin D genes).
...
A prime example of cell cycle being associated with other biological processes is the increased proliferation of activated T cells [@richard2018tcell].
As these data are from CD4+ T-cells, for this reason we are hesitate perform cell cycle adjustment.

# Summary

We have now identified cluster-specific marker genes (i.e. genes that are uniquely upregulated in each cluster compared to all other clusters) and performed a focused analysis using the cyclin genes.
...
<!-- TODO: If James provides cell or cluster-level annotation, I'll add them to the bottom of this report and add to the SCE. -->
