...
```{r}
sce <- readRDS(here("data", "SCEs", "C057_Cooney.merged.SCE.rds"))
```
...
We start from the preprocessed *SingleCellExperiment* object created in ['Merging the Cooney (C057) memory CD4+ T-cell data set'](C057_Cooney_merge.html).
...
```{r}
.adf(dbl_out[, 1:8]) %>%
  knitr::kable(caption = "Results of running `doubletClusters()`.", digits = Inf)
```
...
stopifnot(identical(chosen_doublet, c("11", "5")))
```
...
This does not mean that there aren't doublets; in fact, we strongly suspect that some of the droplets in the 'unknown' HTO cluster are doublets because they are stained with multiple HTOs (see ['Demultiplexing the Cooney (C057) memory CD4+ T-cell data set'](C057_Cooney_demultiplex.html)).
However, these same cells have notably small library sizes (Figure \@ref(fig:library-size-by-hto-cluster)), which means neither `doubletCells()` nor `doubletClusters()` is well-suited to identifying doublets.

```{r library-size-by-hto-cluster, fig.cap = "Library size for each HTO cluster."}
plotColData(sce, x = "hto_cluster", y = "log10_total_counts", colour_by = "hto_cluster") +
...
We can infer that cluster 7 likely has more cells in G2/M than the other clusters, based on higher expression of the cyclin B's.

```{r}
knitr::kable(
  .adf(markers[[7]]),
  caption = "Cyclin DE analysis comparing cluster 7 to all other clusters.")
```
