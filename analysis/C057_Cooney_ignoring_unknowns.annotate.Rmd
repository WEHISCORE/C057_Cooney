...
We can infer that cluster 11 likely has more cells in G2/M than the other clusters, based on higher expression of the cyclin B's.

```{r}
knitr::kable(
  .adf(markers[[11]]),
  caption = "Cyclin DE analysis comparing cluster 11 to all other clusters.")
```
...
Figure \@ref(fig:clusters-by-cyclone-phase) shows an enrichment of cells in G2/M phase in cluster 7 and 11, with cells from the remaining clusters mostly classified as G1.
