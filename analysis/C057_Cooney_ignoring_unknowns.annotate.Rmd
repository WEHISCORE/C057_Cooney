---
title: "Annotating the Cooney (C057) memory CD4+ T-cell data set after ignoring 'unknown' samples"
description: |
author:
  - name: Peter Hickey 
...
knitr::opts_chunk$set(
  fig.path = "C057_Cooney_ignoring_unknowns_annotate_files/")
```
...
```{r}
sce <- readRDS(
  here("data", "SCEs", "C057_Cooney_ignoring_unknowns.merged.SCE.rds"))
```
...
```{r}
dir.create(here("data", "marker_genes", "ignoring_unknowns"))
dir.create(here("output", "marker_genes", "ignoring_unknowns"))
```
...
We start from the preprocessed *SingleCellExperiment* object created in ['Merging the Cooney (C057) memory CD4+ T-cell data set after ignoring the 'unknown' samples'](C057_Cooney_ignoring_unknowns.merge.html).
...
One approach, used here, is to mark a subset of cells (e.g., all cells from one sample) with an antibody conjugated to a different oligonucleotide [@zheng2017massively, @stoeckius2017hashing].
Upon pooling, libraries that are observed to have different oligonucleotides are considered to be doublets and removed.
This approaches can be highly effective but made more difficult for this dataset by one of the samples being labelled with all five HTOs (see ['Demultiplexing the Cooney (C057) memory CD4+ T-cell data set'](C057_Cooney_demultiplex.html)).

A more general approach is to infer doublets from the expression profiles alone [@dahlin2018single].
Here, we will apply two purely computational approaches for detecting doublets from scRNA-seq data.
...
The `doubletCluster()` function identifes from `r BiocStyle::Biocpkg("scran")` clusters with expression profiles lying between two other clusters [@bach2017differentiation].
...
- `N`: The number of genes that are differentially expressed in the same direction in the query cluster compared to both of the source clusters. Such genes would be unique markers for the query cluster and provide evidence against the null hypothesis. Clusters with few unique genes are more likely to be composed of doublets.
- `lib.size` fields: The ratio of the median library size in each source to the median library size in the query. Doublet libraries are generated from a larger initial pool of RNA compared to libraries for single cells, and thus the former should have larger library sizes. Library size ratios much greater than unity are inconsistent with a doublet identity for the query.
- `prop`: The proportion of cells in the query cluster should also be reasonable - typically less than 5% of all cells, depending on how many cells were loaded onto the 10X Genomics device.

For each query cluster, the best pair of sources is identified based solely on the lowest `N`^[This means that any `lib.size` field above unity is not definitive evidence against a doublet identity for a query cluster, as there may be a pair of sources with slightly higher N but both lib.size* values below unity.].
...
```{r}
# TODO: Try overclustering.
# set.seed(46783)
# snn_gr <- buildSNNGraph(sce, use.dimred = "PCA", k = 5)
# clusters <- igraph::cluster_louvain(snn_gr)
# dbl_out <- doubletCluster(sce, factor(clusters$membership))
dbl_out <- doubletCluster(sce, sce$cluster)
```

```{r}
.adf(dbl_out[, 1:8]) %>%
  knitr::kable(
    caption = "Results of running `doubletClusters()`.",
    digits = Inf)
```
...
```{r}
library(scater)
chosen_doublet <- rownames(
  dbl_out)[isOutlier(dbl_out$N, type = "lower", log = TRUE, nmads = 3)]
stopifnot(identical(chosen_doublet, character(0)))
```

Based on the results of running `doubletClusters()`, none of the clusters stand out as being an obvious doublet cluster.

## Doublet detection by simulation

The other doublet detection strategy involves in silico simulation of doublets from the single-cell expression profiles [@dahlin2018single]. 
This is performed using the `doubletCells()` function from the `r BiocStyle::Biocpkg("scran")`, which will:
...
The use of random selection accounts for the relative abundances of different subpopulations, which affect the likelihood of their involvement in doublets; and the calculation of a ratio avoids high scores for non-doublet cells in highly abundant subpopulations.

```{r}
dbl_dens <- doubletCells(sce, BSPARAM = BiocSingular::IrlbaParam())
summary(dbl_dens)
sce$DoubletScore <- log10(dbl_dens + 1)
```

Figures \@ref(fig:tsne-doublet-score) and \@ref(fig:doublet-score) reveal no concentration of the highest doublets scores in a single cluster(s) of cells.

```{r tsne-doublet-score, fig.cap = "t-SNE plot of the data set. Each point is a cell coloured according to its doublet density."}
plotTSNE(sce, colour_by = "DoubletScore")
```

```{r doublet-score, fig.cap = "Distribution of doublet scores for each cluster in the data set. Each point is a cell."}
...
Based on the results of running `doubletClusters()` and `doubletCells()`, none of the clusters stand out as being an obvious doublet cluster.
This does not mean that there aren't doublets; in fact, in ['Demultiplexing the Cooney (C057) memory CD4+ T-cell data set'](C057_Cooney_demultiplex.html) we see evidence of samples with 2 HTOs outside of the 'unknown' cells HTO cluster.
...
saveRDS(
  ...
  here(
    ...
    "marker_genes",
    "ignoring_unknowns",
    "C057_Cooney.markers_union_any.rds"),
  compress = "xz")
dir.create(here("output", "marker_genes", "ignoring_unknowns", "union_any"))
...
Similar figures showing the top 10 marker genes for each cluster are available in [`output/marker_genes/ignoring_unknowns/union_any/heatmaps.union_any.pdf`](../output/marker_genes/ignoring_unknowns/union_any/heatmaps.union_any.pdf).
...
saveRDS(
  ...
  here(
    ...
    "marker_genes",
    "ignoring_unknowns",
    "C057_Cooney.markers_union_up.rds"),
  compress = "xz")
dir.create(here("output", "marker_genes", "ignoring_unknowns", "union_up"))
...
saveRDS(
  ...
  here(
    ...
    "marker_genes",
    "ignoring_unknowns",
    "C057_Cooney.markers_union_up_lfc1.rds"),
  compress = "xz")
dir.create(here("output", "marker_genes", "ignoring_unknowns", "union_up_lfc1"))
...
Similar figures showing the top 10 upregulated and 'upregulated with a log-fold change > 1' marker genes for each cluster are available in [`output/marker_genes/ignoring_unknowns/union_up/heatmaps.union_up.pdf`](../output/marker_genes/ignoring_unknowns/union_up/heatmaps.union_up.pdf). and [`output/marker_genes/ignoring_unknowns/union_up_lfc1/heatmaps.union_up_lfc1.pdf`](../output/marker_genes/ignoring_unknowns/union_up_lfc1/heatmaps.union_up_lfc1.pdf).
...
saveRDS(
  ...
  here(
    ...
    "marker_genes",
    "ignoring_unknowns",
    "C057_Cooney.markers_uniquely_up.rds"),
  compress = "xz")
dir.create(here("output", "marker_genes", "ignoring_unknowns", "uniquely_up"))
...
Similar figures showing the top 100 uniquely upregulated marker genes for each cluster are available in [`output/marker_genes/ignoring_unknowns/uniquely_up/heatmaps.uniquely_up.pdf`](../output/marker_genes/ignoring_unknowns/uniquely_up/heatmaps.uniquely_up.pdf).
...
The tables of marker genes are available as CSV files in [`output/marker_genes/ignoring_unknowns`](../output/marker_genes/ignoring_unknowns) and are named as follows:
...
I use `r Biocpkg("SingleR")` to annotate the data set against 5 reference data sets:

- `HumanPrimaryCellAtlasData`: This reference set entails 713 microarray samples of the Human Primary Cell Atlas (HPCA) [@hpcaRef].
- `BlueprintEncodeData`: This reference set entails 259 RNA-seq samples of pure stroma and immune cells as generated and supplied by Blueprint and ENCODE.
- `DatabaseImmuneCellExpressionData`: This reference set entails 1561 bulk RNA-seq samples of sorted cell populations from the DICE database.
- `MonacoImmuneData`: This reference set entails 114 bulk RNA-seq samples of sorted immune cell populations that can be found in [GSE107011](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE107011).
- `NovershternHematopoieticData`: This reference set entails 211 bulk human microarray samples of sorted hematopoietic cell populations that can be found in [GSE24759](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE24759)
...
The `HumanPrimaryCellAtlasData` reference set entails 713 microarray samples of the Human Primary Cell Atlas (HPCA) [@hpcaRef].
These 713 samples were processed and normalized as described in [@aran2019reference] and each sample has been assigned to one of 37 main cell types and 157 subtypes.
...
The `BlueprintEncodeData` reference set entails 259 RNA-seq samples of pure stroma and immune cells as generated and supplied by Blueprint and ENCODE.
Each sample has a 'main' and a 'fine' cell type label, summarised below.
...
```{r heat-bed-main-clustered, fig.cap = "Heatmap of normalized scores using the main labels from the `BlueprintEncodeData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
```{r heat-bed-main, fig.cap = "Heatmap of normalized scores using the main labels from the `BlueprintEncodeData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_bed_cell_main), colnames(sce)))
plotScoreHeatmap(
  pred_bed_cell_main,
  annotation_col = data.frame(
...
```{r heat-bed-fine-clustered, fig.cap = "Heatmap of normalized scores using the fine labels from the `BlueprintEncodeData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
```{r heat-bed-fine, fig.cap = "Heatmap of normalized scores using the fine labels from the `BlueprintEncodeData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_bed_cell_fine), colnames(sce)))
plotScoreHeatmap(
  pred_bed_cell_fine,
  annotation_col = data.frame(
...
The `DatabaseImmuneCellExpressionData` reference set entails 1561 bulk RNA-seq samples of sorted cell populations from the DICE database.
Each sample has a 'main' and a 'fine' cell type label, summarised below.
...
```{r heat-diced-main-clustered, fig.cap = "Heatmap of normalized scores using the main labels from the `DatabaseImmuneCellExpressionData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
```{r heat-diced-main, fig.cap = "Heatmap of normalized scores using the main labels from the `DatabaseImmuneCellExpressionData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_diced_cell_main), colnames(sce)))
plotScoreHeatmap(
  pred_diced_cell_main,
  annotation_col = data.frame(
...
```{r heat-diced-fine-clustered, fig.cap = "Heatmap of normalized scores using the fine labels from the `DatabaseImmuneCellExpressionData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
```{r heat-diced-fine, fig.cap = "Heatmap of normalized scores using the fine labels from the `DatabaseImmuneCellExpressionData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_diced_cell_fine), colnames(sce)))
plotScoreHeatmap(
  pred_diced_cell_fine,
  annotation_col = data.frame(
...
The `MonacoImmuneData` reference set entails 114 bulk RNA-seq samples of sorted immune cell populations that can be found in [GSE107011](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE107011).
Each sample has a 'main' and a 'fine' cell type label, summarised below.
...
```{r heat-mid-main-clustered, fig.cap = "Heatmap of normalized scores using the main labels from the `MonacoImmuneData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
```{r heat-mid-main, fig.cap = "Heatmap of normalized scores using the main labels from the `MonacoImmuneData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_mid_cell_main), colnames(sce)))
plotScoreHeatmap(
  pred_mid_cell_main,
  annotation_col = data.frame(
...
```{r heat-mid-fine-clustered, fig.cap = "Heatmap of normalized scores using the fine labels from the `MonacoImmuneData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
```{r heat-mid-fine, fig.cap = "Heatmap of normalized scores using the fine labels from the `MonacoImmuneData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_mid_cell_fine), colnames(sce)))
plotScoreHeatmap(
  pred_mid_cell_fine,
  annotation_col = data.frame(
...
The `NovershternHematopoieticData` reference set entails 211 bulk human microarray samples of sorted hematopoietic cell populations that can be found in [GSE24759](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE24759).
...
```{r heat-nhd-main-clustered, fig.cap = "Heatmap of normalized scores using the main labels from the `NovershternHematopoieticData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
plotScoreHeatmap(
  ...
  annotation_col = data.frame(
    ...
    Treatment = sce$Treatment, 
    Sample = sce$Sample,
    row.names = rownames(pred_nhd_cell_main)),
  annotation_colors = list(
    cluster = cluster_colours,
    Treatment = treatment_colours,
    Sample = sample_colours),
...
```{r heat-nhd-main, fig.cap = "Heatmap of normalized scores using the main labels from the `NovershternHematopoieticData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_nhd_cell_main), colnames(sce)))
plotScoreHeatmap(
  pred_nhd_cell_main,
  annotation_col = data.frame(
...
```{r heat-nhd-fine-clustered, fig.cap = "Heatmap of normalized scores using the fine labels from the `NovershternHematopoieticData` reference set. Cells are columns, rows are labels. Columns are ordered by clustering the normalized scores.", fig.asp = 0.9, layout = "l-body-outset"}
...
plotScoreHeatmap(
  ...
  annotation_col = data.frame(
    ...
    Treatment = sce$Treatment, 
    Sample = sce$Sample,
    row.names = rownames(pred_nhd_cell_fine)),
  annotation_colors = list(
    cluster = cluster_colours,
    Treatment = treatment_colours,
    Sample = sample_colours),
...
```{r heat-nhd-fine, fig.cap = "Heatmap of normalized scores using the fine labels from the `NovershternHematopoieticData` reference set. Cells are columns, rows are labels. Columns are ordered by and annotated by the column labels.", fig.asp = 0.9, layout = "l-body-outset"}
stopifnot(identical(rownames(pred_nhd_cell_fine), colnames(sce)))
plotScoreHeatmap(
  pred_nhd_cell_fine,
  annotation_col = data.frame(
...
For example, Figure \@ref(fig:cyclin-heatmap) shows that cluster 7 may be in G2/M phase while the other clusters are scattered across the later phases.
...
We can infer that cluster 10 likely has more cells in G2/M than the other clusters, based on higher expression of the cyclin B's.

```{r}
knitr::kable(
  .adf(markers[[10]]),
  caption = "Cyclin DE analysis comparing cluster 10 to all other clusters.")
```
...
Figure \@ref(fig:clusters-by-cyclone-phase) shows an enrichment of cells in G2/M phase in cluster 9, with cells from the remaining clusters mostly classified as G1.
...
However, it is not difficult to find routine violations of this assumption.
...
The processed *SingleCellExperiment* object is available (see [`data/SCEs/C057_Cooney_ignoring_unknowns.annotated.SCE.rds`](../data/SCEs/C057_C057_Cooney_ignoring_unknowns.annotated.SCE.rdsCooney.annotated.SCE.rds)).
